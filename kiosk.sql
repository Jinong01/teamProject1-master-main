...
create table orders(
    ...
    pay_method int comment '결제 수단 : 1.현금 2.신용 카드 3.삼성 4.카카오톡',
    pay_status int comment '결제 상태: 1.yes 2.no',
...
create table order_detail (
    ...
    order_id int  comment '주문 아이디',
    dish_id int  comment '메뉴 아이디',
    number int  comment '갯수',
...
create table shopping_cart(
    ...
    temperature varchar(10) comment '온도',
    shot varchar(10) comment '샷',
    foreign key (user_id) references user(id),
    foreign key (dish_id) references dish(id)
)comment '장바구니';
